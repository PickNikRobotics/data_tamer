...
- **§10.1 steps 1–3** (micro-benchmarks for the mutex and pool, re-run after `ValuePtr`): the mutex/pool micro-benchmarks are covered by the unit tests' timing-free assertions and by Task 8 step 5's re-run; dedicated Google Benchmark cases for `WriteMutex` and `SnapshotPool` are deferred to Plan 2's first task so they measure the wired code paths rather than isolated calls.
