...
| Deterministic sink-unpublication overlap test | **Adopted.** | `ChannelControl.RemovalWaitsForPausedReaderAndReferencesSurvive`: the existing paused serializer parks the reader *inside* its epoch, after loading the sink links and before pushing. Removal is then observed not completing within 50 ms, and the push lands on the still-alive unpublished link. No production hook and no procfs needed. Mutation check: deleting `waitQuiescent()` from `removeDataSink()` makes the test fail on the first run. |
